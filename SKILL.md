...
检测器永远分不清、但确实是有意为之的写法（前后端镜像、一次性迁移、验证幂等的重复初始化），不要长期躺在基线里：先尽量提升检测精度（见 `references/guard-catalog.md` 各节的「误报」），剩下的用写在代码旁边的有意保留标记 `// guard-allow(<守卫名>): <理由>`，语义见 `references/contract.md`。基线条目一挪位置就报新增，标记跟着代码走。
...
向用户说明：
- 加了哪些守卫、各自的命令、哪些是硬规则
- 每个基线里有多少条，挑几条有代表性的说明它们是真问题还是误报；用了有意保留标记的，列出位置和理由
- 已知误报来源和打算怎么处理
- 为了让硬规则当天通过改了哪些非业务文件
